...
T = np.full((NUM_STATES, NUM_ACTIONS, NUM_STATES), 1/NUM_STATES)
Rs = np.zeros(NUM_STATES)

s_a_ns_counter = np.zeros_like(T)  # counters for every time state-action-newstate was encountered
ns_reward_counter = np.zeros(NUM_STATES)  # counters for the total rewards for each new state
ns_counter = np.zeros(NUM_STATES)  # counters for the number of times we encountered a state
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    # model.
    ###### BEGIN YOUR CODE ######
    actions_sum = np.zeros(NUM_ACTIONS)
    for i in range(NUM_ACTIONS):
        actions_sum[i] = np.dot(T[state][i] , V)
    action = np.argmax(actions_sum)
    ###### END YOUR CODE ######
    ...
    ###### BEGIN YOUR CODE ######
    s_a_ns_counter[state][action][new_state] += 1
    ns_reward_counter[new_state] += R
    ns_counter[new_state] += 1
    ...
    if new_state == NUM_STATES - 1:
        ...
        for s in range(NUM_STATES):
            for a in range(NUM_ACTIONS):
                total_encounters = np.sum(s_a_ns_counter[s][a])
                if total_encounters != 0:
                    for ns in range(NUM_STATES):
                        T[s][a][ns] = s_a_ns_counter[s][a][ns] / total_encounters


            if ns_reward_counter[s] != 0:
                Rs[s] = ns_reward_counter[s] / ns_counter[s]
        ...
        while 1:
            for s in range(NUM_STATES):
                actions_sum = np.zeros(NUM_ACTIONS)
                for a in range(NUM_ACTIONS):
                    actions_sum[a] = np.dot(T[s][a], V)
                new_val[s] = Rs[s] + GAMMA * np.max(actions_sum)

            # check if we should break
            diff = np.absolute(new_val - V)
            V = new_val
            if np.amax(diff) < TOLERANCE:
                consecutive_no_learning_trials = 0 if iter_count != 0 else consecutive_no_learning_trials + 1
                break
            else:
                iter_count += 1
